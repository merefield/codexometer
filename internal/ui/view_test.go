...
func TestPricingFooterCentersProvenanceOnEveryPricedView(t *testing.T) {
	...
	pricedViews := append([]meterStyleID{styleBenchmark}, quotaStyleOrder[:]...)
	for _, style := range pricedViews {
		model.meterStyle = style
		raw := model.renderFooter(100, colors)
		if !strings.Contains(raw, codex.StandardAPIPricingSourceURL) {
			t.Errorf("%s footer does not contain pricing hyperlink: %q", style.name(), raw)
			continue
		}
		...
		if start < 0 {
			t.Errorf("%s footer does not contain pricing provenance: %q", style.name(), firstLine)
			continue
		}
		center := start + len(label)/2
		if center < 49 || center > 51 {
			t.Errorf("%s pricing provenance center = %d, want terminal center 50: %q", style.name(), center, firstLine)
		}
	}

	for _, style := range []meterStyleID{styleMonitor} {
		model.meterStyle = style
		...
		for _, width := range []int{12, 24, 40, 60, 79, 80, 100} {
			...
			for line, value := range strings.Split(footer, "\n") {
				if got := lipgloss.Width(value); got > width {
					t.Errorf("%s width %d line %d rendered at %d: %q", style.name(), width, line, got, ansi.Strip(value))
				}
			}
			hasPricing := strings.Contains(footer, codex.StandardAPIPricingSourceURL)
			if width < 80 && hasPricing {
				t.Errorf("%s width %d unexpectedly showed pricing provenance", style.name(), width)
			}
			if width >= 80 && !hasPricing {
				t.Errorf("%s width %d omitted responsive pricing provenance", style.name(), width)
			}
		}
	}
}
