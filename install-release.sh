...
repository=${CODEXOMETER_REPOSITORY:-merefield/codexometer}
release_tag=${CODEXOMETER_VERSION:-latest}
bin_dir=${CODEXOMETER_BIN_DIR:-/usr/local/bin}
binary_name=${CODEXOMETER_BIN_NAME:-codexometer}
github_url=${CODEXOMETER_GITHUB_URL:-https://github.com}
github_api_url=${CODEXOMETER_GITHUB_API_URL:-https://api.github.com}

usage() {
  cat <<'EOF'
Install a pre-built Codexometer release from GitHub.
...
Environment:
  CODEXOMETER_VERSION         Release tag to install (default: latest).
  CODEXOMETER_BIN_DIR         Installation directory (default: /usr/local/bin).
  CODEXOMETER_BIN_NAME        Installed binary name (default: codexometer).
  CODEXOMETER_REPOSITORY      GitHub owner/repository (default: merefield/codexometer).
  CODEXOMETER_GITHUB_URL      GitHub web base URL (primarily for testing/mirrors).
  CODEXOMETER_GITHUB_API_URL  GitHub API base URL (primarily for testing/mirrors).
EOF
}

fail() {
  printf 'codexometer installer: %s\n' "$*" >&2
  exit 1
}
...
case "$repository" in
  /*|*/|*//*|*[!A-Za-z0-9._/-]*) fail "invalid CODEXOMETER_REPOSITORY: $repository" ;;
esac
case "$repository" in
  */*) ;;
...
[ -n "$bin_dir" ] || fail "CODEXOMETER_BIN_DIR must not be empty"
if { [ -e "$bin_dir" ] || [ -L "$bin_dir" ]; } && [ ! -d "$bin_dir" ]; then
  fail "CODEXOMETER_BIN_DIR exists and is not a directory: $bin_dir"
fi
case "$binary_name" in
  ''|*/*) fail "CODEXOMETER_BIN_NAME must be a single file name" ;;
esac
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/codexometer-release-install.XXXXXX")
trap 'rm -rf "$tmp_dir"' EXIT HUP INT TERM

if [ "$release_tag" = latest ]; then
  printf 'Resolving the latest Codexometer release...\n'
...
archive_name=codexometer_${release_version}_${release_os}_${release_arch}.tar.gz
release_url=${github_url}/${repository}/releases/download/${release_tag}
archive_path=${tmp_dir}/${archive_name}
checksums_path=${tmp_dir}/checksums.txt

printf 'Downloading Codexometer %s for %s/%s...\n' "$release_tag" "$release_os" "$release_arch"
...
member_count=$(tar -tzf "$archive_path" | awk '$0 == "codexometer" { count++ } END { print count + 0 }')
[ "$member_count" -eq 1 ] || fail "release archive does not contain exactly one root-level codexometer binary"
extract_dir=${tmp_dir}/extract
mkdir "$extract_dir"
tar -xzf "$archive_path" -C "$extract_dir" codexometer
candidate=${extract_dir}/codexometer
[ -x "$candidate" ] || fail "the downloaded codexometer binary is not executable"

version_output=$("$candidate" --version 2>&1) || fail "the downloaded codexometer binary failed its version check"
[ "$version_output" = "codexometer $release_version" ] ||
